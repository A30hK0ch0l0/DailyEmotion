...
from daily_emotions_instagram.parameters import emotion_mean_dic,  model_dic
...
def days3(data: pd.DataFrame):
    ...
    for i in range(3):
        start_time1 = (datetime.datetime(int(end_time[:4]), int(end_time[5:7]), int(end_time[8:])) + datetime.timedelta(
            days=i + 1)).date()
    ...
    est_emotion = ["anger_est", "wonder_est", "fear_est", "hate_est", "expectation_est", "sadness_est",
                   "happiness_est", "credence_est", "hope_est"]
    ...
    for em in est_emotion:
        ...
        for i in range(3):
            ...
            X_test = K.cast_to_floatx(ohlcv_test)
